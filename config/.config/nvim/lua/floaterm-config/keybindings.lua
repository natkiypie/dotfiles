...
keys.bind_normal_mode {
  ...
  { '<C-g>', '<CMD>lua require"floaterm-config.utils".toggle{name="LazyGit", cmd="lazygit"}<CR>' },
  { '<C-a>', '<CMD>lua require"floaterm-config.utils".toggle{name="Glow", cmd="glow_script.sh"}<CR>' },
  {
    '<F7>',
    '<CMD>lua require"floaterm-config.utils".toggle{wintype="split", height="0.5", name="htop", cmd="htop -p `pidof vim`"}<CR>',
  },
